...
class concorsoGenericDao {
    ...
    async initializeDatabase() {
        try {
            ...
            console.log(`Connected to MongoDB ${this.concorso} database ${databaseConfig.dbURI}`);
            this.db = this.mongoClient.db(databaseConfig.dbName);
            this.candidatiCollection = this.db.collection(`${this.concorso}`); 
        } catch (error) {
    ...
    async getAmmessiSortedByCognomeAndNome() {
        try {
            const result = await this.candidatiCollection.find({ statoCandidato: "AMMESSO" })
                .sort({ cognome: 1, nome: 1 })
                .toArray();
            return result;
        } catch (error) {
            console.error('Error fetching ammessi candidati:', error);
            throw error;
        }
    }
    
    async getAllDocumentsWithLastDomandaConcorso() {
        try {
            const result = await this.candidatiCollection.aggregate([
                {
                    $project: {
                        codiceFiscale: 1,
                        cognome: 1,
                        ...
                        statoDomanda: 1,
                        statoCandidato: 1,
                        dataUltInvioDomanda: 1,
                        annullaDomanda: 1,
                        domandeConcorso: { $arrayElemAt: ["$domandeConcorso", -1] },
                        iterConcorso: 1
                    }
                },
                {
    ...
    async getAllCNVVFCandidates() {
        try {
            const result = await this.candidatiCollection.aggregate([
                {
                    $project: {
                        ...
                        dataUltInvioDomanda: 1,
                        annullaDomanda: 1,
                        lastDomandaConcorso: { $arrayElemAt: ["$domandeConcorso", -1] },
                        iterConcorso: 1
                    }
                },
                {
                    $match: {
                        'lastDomandaConcorso.appartenenteCNVVF': true
                    }
                },
                {
                    $sort: { cognome: 1, nome: 1 }
            ...
            console.error('Error fetching CNVVF candidates:', error);
            throw error;
        }
    }
    async getAllInvaliditaCivileCandidates() {
        try {
            const result = await this.candidatiCollection.aggregate([
                {
                    $project: {
                ...
                {
                    $match: {
                        'lastDomandaConcorso.invaliditaCivile': { $ne: null }
                    }
                },
                {
                    $sort: { cognome: 1, nome: 1 }
            ...
            console.error('Error fetching invaliditaCivile candidates:', error);
            throw error;
        }
    }
    async getAllDSACandidates() {
        try {
            const result = await this.candidatiCollection.aggregate([
                {
                    $project: {
                ...
                {
                    $match: {
                        'lastDomandaConcorso.dSA': { $ne: null }
                    }
                },
                {
                    $sort: { cognome: 1, nome: 1 }
            ...
            console.error('Error fetching DSA candidates:', error);
            throw error;
        }
    }
...
module.exports = concorsoGenericDao;
