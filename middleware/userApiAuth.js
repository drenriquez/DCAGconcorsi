...
async function userApiAuth (req, res, next) {
    try {
        ...
        if (req.path === '/concorsi/login') {
            console.log('Verifica se la richiesta è per la pagina di login,req.path === /concorsi/login')
            return next();
        }
        ...
        if (!isSessionOk) {
            ...
            console.log("----------------Sessione non Trovata (middlware userApiAuth) NON AUTORIZZATO --------------",req.path);
